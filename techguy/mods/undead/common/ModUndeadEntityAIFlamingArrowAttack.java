...
public class ModUndeadEntityAIFlamingArrowAttack extends EntityAIBase
{
    World worldObj;
    ...
    public ModUndeadEntityAIFlamingArrowAttack(EntityLiving par1EntityLiving, float par2, int par3, int par4)
    {
        rangedAttackTime = 0;
        field_48367_f = 0;
    ...
    private void doRangedAttack()
    {
        if (rangedAttackID == 1)
        {
        	ModUndeadEntityFlamingArrow entityflamingarrow = new ModUndeadEntityFlamingArrow(worldObj, entityHost, attackTarget, 1.6F, 12F);
            worldObj.playSoundAtEntity(entityHost, "random.bow", 1.0F, 1.0F / (entityHost.getRNG().nextFloat() * 0.4F + 0.8F));
            worldObj.spawnEntityInWorld(entityflamingarrow);
            entityflamingarrow.setFire(5);
        }
    }
}
